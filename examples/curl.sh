#!/usr/bin/env bash
# Claude Opus 5 (claude-opus-5): non-streaming, streaming and tool-calling request shapes.
set -euo pipefail
...
curl -sS "$BASE/chat/completions" "${AUTH[@]}" \
  -d '{"model":"claude-opus-5","stream":false,"messages":[{"role":"user","content":"Name three retry rules, one per line."}]}' \
...
curl -sS "$BASE/chat/completions" "${AUTH[@]}" \
  -d '{"model":"claude-opus-5","stream":true,"messages":[{"role":"user","content":"Count to five."}]}' \
  | head -c 400; echo

echo
echo "== tool calling =="
curl -sS "$BASE/chat/completions" "${AUTH[@]}" \
  -d '{"model":"claude-opus-5","stream":false,"messages":[{"role":"user","content":"What is the weather in Paris? Use the tool."}],
